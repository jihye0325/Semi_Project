			...
			<div class="notice_area">
				...
				<div class="member_list">
					...
					<c:forEach var="r" items="${roomList}">
						...
						<li class="name">${r.r_name}</li>
						<li class="tel">${r.r_tel}</li>
						<li class="address">${r.r_address}</li>
						<li><button type = "button" onclick="deleteRoom()">삭제</button></li>
				...
				<ul class="board_paging">
					<li><a href="${contextPath}/admin/roomAllList?page=1">&lt;&lt;</a></li>
					<li><c:choose>
							<c:when test="${pi.page>1 }">
								<a href="${contextPath}/admin/roomAllList?page=${pi.page-1}">&lt;</a>
							</c:when>
							<c:otherwise>
								<a href="#">&lt;</a>
							</c:otherwise>
					...
					<c:forEach var="p" begin="${pi.startPage }" end = "${pi.endPage }">
						<li><c:choose>
								...
								<c:otherwise>
									<a href="${contextPath }/admin/roomAllList?page=${p}">${ p }</a>
								</c:otherwise>
							</c:choose></li>
					</c:forEach>
					...
					<li><a href="${contextPath}/admin/roomAllList?page=${pi.maxPage}">&gt;&gt;</a></li>
				</ul>
			</div>
			<div class="search_area">
				<form method="get" action="${contextPath }/admin/roomAllList">
					<select id="searchCondition" name="searchCondition">
						<option value="s_name" <c:if test ="${param.searchCondition == 'r_name' }">selected</c:if>>숙소 이름</option>
					</select>
...
<script>
			function deleteRoom(){
				if(confirm('삭제하시겠습니까?')){
				document.forms.roomForm.action="${contextPath}/admin/deleteRoom";
