...
<body>
	...
	<div class="outer">
		...
		<div class="wrap">
			<div class="notice_area">
				...
				<div class="member_list">
					<ul class="memberhead_ul">
						...
						<li class="authority">권한</li>
					</ul>
					<c:forEach var="m" items="${memberList }">
						<ul class="member_ul" onclick="detailView(${ m.user_no});">
						
							<li class="no">${ m.user_no }</li>
							<li class="id">${ m.id }</li>
							<li class="userName">${ m.user_name }</li>
							<li class="authority">
								<c:set var="authority" value="${m.authority }"/> 
									<c:choose>
										<c:when test="${m.authority eq 1}">
											<a>게스트</a>
										</c:when>
										<c:when test="${m.authority eq 2}">
											<a>호스트</a>
										</c:when>
										<c:when test="${m.authority eq 3}">
											<a>관리자</a>
										</c:when>
										<c:otherwise>
											<a>없음</a>
			...
			<c:if test="${!empty param.searchCondition && !empty param.searchValue }">
					...
					<li><a href="${contextPath}/admin/memberManagement?page=1${searchParam}">&lt;&lt;</a></li>
					<li><c:choose>
							<c:when test="${pi.page>1 }">
								<a href="${contextPath}/admin/memberManagement?page=${pi.page-1}${searchParam}">&lt;</a>
							</c:when>
							<c:otherwise>
								<a href="#">&lt;</a>
							</c:otherwise>
					...
					<c:forEach var="p" begin="${pi.startPage }" end = "${pi.endPage }">
						<li><c:choose>
								...
								<c:otherwise>
									<a href="${contextPath }/admin/memberManagement?page=${p}${searchParam}">${ p }</a>
								</c:otherwise>
							</c:choose></li>
					</c:forEach>
					...
					<li><a href="${contextPath}/admin/memberManagement?page=${pi.maxPage}${searchParam}">&gt;&gt;</a></li>
				</ul>
				<div class="search_area">
				<form method="get" action="${contextPath }/admin/memberManagement">
					<select id="searchCondition" name="searchCondition">
						<option value="username" <c:if test ="${param.searchCondition == 'user_name' }">selected</c:if>>이름</option>
	...
	<script>
		...
		//회원 상세정보 페이지
		function detailView(user_no) {
		
			location.href='${contextPath}/admin/memberdetail?user_no='+user_no;
